...
var binary = this;
this.BinaryParser = function(bigEndian, allowExceptions){
    this.bigEndian = bigEndian, this.allowExceptions = allowExceptions;
};
with({p: this.BinaryParser.prototype}){
...
this.Reader = function(buffer, bigEndian) {
	var position = 0,
	    parser = new binary.BinaryParser(bigEndian, true),
	    that = this;
	    
	var getBits = function(amount) {
		return that.getBuffer(Math.ceil(amount/8)).toString('binary');
	}
	...
	this.getBuffer = function(amount) {
		var end = Math.min(buffer.length, position + amount);
		var slice = buffer.slice(position, end);
		position = end;
		return slice;
	}
	
	this.getInt = function(bits, signed) {
		return parser.decodeInt(getBits(bits), bits, signed);
	}
	
	this.getFloat = function(precisionBits, exponentBits) {
		return parser.decodeFloat(
			getBits(precisionBits + exponentBits),
			precisionBits,
			exponentBits
		);
	}
	
	this.getUTF8 = function(amount) {
		return this.getBuffer(amount).toString('utf8');
	}
}
